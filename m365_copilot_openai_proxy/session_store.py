"""Persistent Copilot sessions, keyed so multiple clients can each keep a
stable conversation across turns.

Uses a threading lock per session (the server is synchronous and threaded),
replacing the asyncio lock from the original.
"""
