...
_RECV_TIMEOUT = 120.0  # seconds to wait between Copilot frames before giving up
...
class SubstrateCopilotClient:
    def __init__(self, access_token: str, time_zone: str = "Asia/Tokyo"):
        if not access_token:
            raise SubstrateCopilotError(
                ...
                "or run `python -m m365_copilot_openai_proxy set-token`."
            )
    ...
    def chat_stream(
        ...
        if session is None:
            yield from self._chat_stream_for_turn(
                text=text,
    ...
    def _chat_stream_for_turn(
        ...
        try:
            ws.send(json.dumps({"protocol": "json", "version": 1}) + SIGNALR_SEP)
            ws.recv(timeout=_RECV_TIMEOUT)  # SignalR handshake ack
            ws.send(self._chat_invoke(text, conv_id, session_id, req_id, is_start_of_session))
            fallback_text = ""
            yielded_any = False
            while True:
                try:
                    ...
                    # Clean close or read timeout: end of stream.
    ...
    def chat(
        ...
        return "".join(self.chat_stream(prompt, additional_context, session))
