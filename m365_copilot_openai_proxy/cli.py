...
from .wsclient import WebSocket, WebSocketError, WebSocketTimeout

try:
    import msvcrt  # Windows-only stdlib; used for the interactive serve console
...
def _ws_read_until_id(ws: WebSocket, want_id: int, timeout: float):
...
# --- CDP token extraction/capture (replaces async websockets/httpx) --------

def _cdp_extract_token(port: int, *, allow_nudge: bool = True) -> str | None:
    try:
        tabs = _http_get_json(f"http://localhost:{port}/json", timeout=1)
    except Exception:
        return None
    ...
    try:
        with WebSocket.connect(tab["webSocketDebuggerUrl"], proxy="") as ws:
            ...
            if not allow_nudge:
                return None
            return _cdp_nudge_and_wait_for_token(ws)
    except Exception:
        return None


def _cdp_capture_websocket_token(port: int, timeout_seconds: int) -> str | None:
    deadline = time.time() + timeout_seconds
    while time.time() < deadline:
        try:
            tabs = _http_get_json(f"http://localhost:{port}/json", timeout=3)
        except Exception:
            time.sleep(1)
            continue
...
def _wait_for_substrate_websocket_token(ws: WebSocket, deadline: float) -> str | None:
    while time.time() < deadline:
        try:
...
def _cdp_nudge_and_wait_for_token(ws: WebSocket) -> str | None:
    ws.send(json.dumps({"id": 2, "method": "Network.enable"}))
    ws.send(json.dumps({"id": 3, "method": "Runtime.evaluate", "params": {"expression": _CDP_NUDGE_JS}}))
    ws.send(json.dumps({"id": 4, "method": "Input.insertText", "params": {"text": " "}}))
    ws.send(json.dumps({
        "id": 5,
    ...
    return _wait_for_substrate_websocket_token(ws, time.time() + 10)
...
def _wait_for_m365_page(cdp_port: int, timeout_seconds: int) -> bool:
    ...
    while time.time() < deadline:
        try:
            tabs = _http_get_json(f"http://localhost:{cdp_port}/json", timeout=1)
        except Exception:
            time.sleep(0.5)
            continue
...
def _capture_token_to_env(cdp_port: int, timeout_seconds: int) -> bool:
    token = _cdp_capture_websocket_token(cdp_port, timeout_seconds)
    if not token:
        return False
    _write_token(token)
    return True
...
def _try_auto_refresh(cdp_port: int, *, allow_nudge: bool = True) -> bool:
    token = _cdp_extract_token(cdp_port, allow_nudge=allow_nudge)
    if not token:
        return False
    _write_token(token)
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="m365-copilot-openai-proxy")
...
def capture_token_command(args: argparse.Namespace) -> None:
    ...
    token = _cdp_capture_websocket_token(args.cdp_port, args.timeout_seconds)
    if not token:
...
def serve_command(args: argparse.Namespace) -> None:
    ...
    while True:
        ...
        if msvcrt is not None:
            ...
            print(
                f"  auto-refresh: {auto_refresh_label}"
                f"    startup-capture: {capture_label}    (Ctrl-C to quit)\n"
            )

        action = _serve_console_loop(thread, server)
...
def _serve_console_loop(thread: threading.Thread, server: Server) -> str | None:
    """Return 'quit', 'refresh', or None. Uses msvcrt keys on Windows; falls
    back to blocking until Ctrl-C elsewhere."""
    if msvcrt is None:
        try:
